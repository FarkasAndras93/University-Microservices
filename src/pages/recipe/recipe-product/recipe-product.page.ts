...
export class RecipeProductPage {
  ...
  constructor(private toast: ToastProvider, public viewCtrl: ViewController, private storage: StorageProvider,
  ...
  ionViewDidLoad() {
    ...
    if (!GlobalUtils.isUndefinedOrNull(this.navParams.get("products") && this.navParams.get("products").length > 0)) {
      this.navParams.get("products").forEach(product => {
        this.possibleProducts.push(new RecipeProduct(true, product));
      });
    }
    this.productProvider.getAllProducts().then((products) => {
      products.forEach(product => {
        if (GlobalUtils.isUndefinedOrNull(this.possibleProducts.filter(function (obj) { return obj.myProduct.product.name == product.name })[0])) {
          this.possibleProducts.push(new RecipeProduct(false, new MyProduct(undefined, product,
            this.storage.getLoggedUser(), undefined, undefined, MYPRODUCT_TYPE.RECIPE)));
        }
      });
    }).catch(error => {
  ...
  public addProducts() {
    ...
    if (products.filter(
      product => {
        if (GlobalUtils.isUndefinedOrNull(product.myProduct.weight) || product.myProduct.weight <= 0) {
          return true;
        } else {
          return false;
        }
