...
//         console.log(product);
//         if (GlobalUtils.isUndefinedOrNull(this.navParams.get("existentProducts")) ||
//           this.navParams.get("existentProducts").filter(posProd => posProd.myProduct.product.name == product.name).length <= 0) {
//           this.possibleProducts.push(new GroceryProduct(0, new MyProduct(0, product, this.storage.getLoggedUser(), undefined, 0, MYPRODUCT_TYPE.GROCERY), false));
//         }
//       });
//     }).catch(error => {
//       console.error("Error while returning all products.", error);
//     });
